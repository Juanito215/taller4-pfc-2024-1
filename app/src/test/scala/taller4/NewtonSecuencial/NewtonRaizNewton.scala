...
class NewtonRaizNewton extends AnyFunSuite{
  ...
  test("raizNewtonTest1") {
    ...
    val raiz = e.raizNewton(expr, e.Atomo('x'), 2.0, buenaAprox)
  ...
  test("raizNewtonTest2") {
    ...
    val raiz = e.raizNewton(expr, e.Atomo('x'), 1.0, buenaAprox)
  ...
  test("raizNewtonTest4"){
    ...
    val raiz = e.raizNewton(expr, e.Atomo('x'), 4.0, buenaAprox)
    assert(buenaAprox(expr, e.Atomo('x'), raiz))
  }
  test("raizNewtonTest5"){
    val expr = e.Resta(e.Prod(e.Atomo('x'), e.Logaritmo(e.Atomo('x'))), e.Numero(3.0))
    val raiz = e.raizNewton(expr, e.Atomo('x'), 15.0, buenaAprox)
    assert(buenaAprox(expr, e.Atomo('x'), raiz))
  }
